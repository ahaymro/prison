...
<div style="width: 850px; height: 300px; overflow: auto;">
           <div>
            <p style="font-family: lucida calligraphy; text-align: center;">1.green background==today's new<br>
            2.HoneyDew background==current new's</p>
...
try {
    Class.forName("com.mysql.jdbc.Driver");
   con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");
    String sql = "SELECT * FROM news ORDER BY news.date DESC";
    ps = con.prepareStatement(sql);
    rs = ps.executeQuery();
    int report = 0;

    out.println("<table class='table table-bordered' style='margin-top:20px;'><tr><th style='text-align:center;'>Date</th> <th style='text-align:center;'>Description</th></tr>");
    ...
    while (rs.next()) {
        ...
        if (date.equals(new java.sql.Date(new java.util.Date().getTime()).toString())) {
            out.println("<tr style='background-color:green; color:black;'><td>" + date + "</td><td>" + description + "</td></tr>");
        } else {
            out.println("<tr style='background-color:HoneyDew;'><td>" + date + "</td><td>" + description + "</td></tr>");
        }
    }

    if (report == 0) {
        out.println("new information is not posted until now");
    }
