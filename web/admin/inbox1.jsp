...
try {
    ...
    if (request.getParameter("create") != null) {
        ...
        if (fname.isEmpty() || mname.isEmpty() || serial_no.isEmpty()) {
            out.println("<font style='position:absolute; top:650px; left:900px;color:red; font-size:20px;'>please fill the form</font>");
        } else {
            if (password.equals(password1)) {
                query = "INSERT INTO account(id, ssn, username, password, zone, account_type, status) VALUES (?,?,?,?,?,?,?)";
                ...
                if (rowsAffected > 0) {
                    if (account_type.equals("comissioner")) {
                        query = "INSERT INTO commisioner_information(fname, ssn, lname, sex, age, zone) VALUES (?,?,?,?,?,?)";
                        pstmt = con.prepareStatement(query);
                        ...
                        if (rowsAffected > 0) {
                            out.println("<font style='position:absolute; top:650px; left:900px;color:red; font-size:20px;'>successfully created</font>");
                        } else {
                            out.println("<font style='position:absolute; top:650px; left:900px;color:red; font-size:20px;'>Error Occurred! May be due to<br> serial no,username,and password duplication</font>");
                        }
                    } else if (account_type.equals("inspector")) {
                        query = "INSERT INTO inspector_information(fname, lname, ssn, sex, age, zone) VALUES (?,?,?,?,?,?)";
                        pstmt = con.prepareStatement(query);
                        pstmt.setString(1, fname);
                ...
                out.println("<font style='position:absolute; top:650px; left:900px;color:red; font-size:20px;'>confirmation password <br>is incorrect try again!</font>");
            }
        }
    }
...
<ul class="dropbtn" style="background-color: gray;">
 ...
 con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");
  st=con.createStatement();
  String sql2="select count(id) as total  from admin_message where status=0";
...
<table class="table table-bordered ">
  ...
  Class.forName("com.mysql.jdbc.Driver");
 con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");
String sql1="select * from admin_message where status=0";
...
<table class='table table-bordered'>
    ...
    <tr>
        ...
        <td><a href='read.jsp?id=<%=rs.getInt("id")%>' style='color:red; font-size:20px;'>seen</a></td>
    </tr>
    <%
    }
    %>
