...
<table class="table table-bordered ">
  <tr>
    ...
    <tr style="text-align: center;"><td style="font-size: 20px; background-color: LightSteelBlue; color: white; font-family: comic sans Ms;">do you know about BG</td>
        <td style="font-size: 20px; background-color: LightSteelBlue; color: black; font-family: comic sans Ms;" >show activate account</td>
 ...
 con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");  
 st=con.createStatement();
String sql3 = "select * from account where status=1";
...
if (rs.next()) {
    do {
        ...
        <tr>
            ...
            <td><a href="deactivated.jsp?id=<%= rs.getInt("id") %>">deactivate</a></td>
        </tr>
        <%
    } while (rs.next());
} else {
...
<table><tr>
  <%
   Class.forName("com.mysql.jdbc.Driver");
 con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");  
String serial_no = (String) sessio.getAttribute("ssn");
String sql4 = "select * from admin_info where ssn='" + serial_no + "'";
...
if (request.getParameter("update") != null) {
  ...
  if (newPwd.equals(copassword)) {
    Class.forName("com.mysql.jdbc.Driver");
     con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");
     st = con.createStatement();
    String sql5="SELECT * FROM account WHERE username='"+username+"' AND password='"+current+"'";
    ...
    if (report>0) {
      String sql6="UPDATE account SET password='"+newPwd+"' WHERE username='"+username+"'";
      int query=st.executeUpdate(sql6);
      if (query>0) {
