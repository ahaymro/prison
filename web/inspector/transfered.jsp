...
String serialno = (String) sessio.getAttribute("ssn");
...
if (request.getParameter("update") != null) {
    String transferto = request.getParameter("transferto");
     ...
     Statement  st1 = con.createStatement();
            String sql = "SELECT  * FROM prisoner_information WHERE ssn='"+request.getParameter("id")+"'";
          ResultSet   rs1 = st1.executeQuery(sql);
          if(rs1.next()){
    if (transferto.equals(rs1.getString("zone"))) {
        out.println("<script>alert('Already Exists In This Zone')</script>");
    } 
          
          else {
          st = con.createStatement();
            String sql1 = "SELECT zone FROM prisoner_information WHERE ssn='"+request.getParameter("id")+"'";
           rs = st.executeQuery(sql1);
               if(rs.next()){
         
                String query1 = "UPDATE prisoner_information SET zone=? WHERE ssn=?";
                ps = con.prepareStatement(query1);
                ps.setString(1, transferto);
                ps.setString(2, request.getParameter("id"));
                int result1 = ps.executeUpdate();
                if (result1 > 0) {
                    String query2 = "INSERT INTO prisoner_message(ssn, sender, reciver, message, Date, status,Reason) VALUES (?,?,?,?,?,?,?)";
                   
                    ps = con.prepareStatement(query2);
                    ps.setString(1, request.getParameter("id"));
                    ps.setString(2, "inspector");
                      ps.setString(3, "policeofficer");
                    ps.setString(4, "you are transfered from " + rs.getString("zone") + " to " + transferto);
                    ps.setString(5, formattedDate);
                    ps.setInt(6, 0);
                   ps.setString(7, Reason);
                    int result2 = ps.executeUpdate();
                    if (result2 > 0) {
                        out.println("<script>alert('Successfully transferred')</script>");
                    } else {
                       out.println("<script>alert('Not transferred')</script>");
                    }
                } else {
                    out.println("<script>alert('Not transferred')</script>");
                }
...
<table class="table table-bordered ">
  ...
  <tr>
    ...
    <tr style="text-align: center;"><td style="font-size: 20px; background-color: LightSteelBlue; color: white; font-family: comic sans Ms;">do you know about BG</td>
        <td style="font-size: 20px; background-color: LightSteelBlue; color: black; font-family: comic sans Ms;" >Transfer Prisoner page</td>
        <td style="font-size: 20px; background-color: LightSteelBlue; color: white; font-family: comic sans Ms;"></td></tr>
    <tr><td class="text-justify" style="font-size: 14px; width: 300px;">  Benishangul-Gumuz also known as Benshangul/Gumuz, is one of the nine ethnic divisions (kililoch) of Ethiopia. It was previously known as Region 6.The region's capital is Assosa<br><br>
    ...
    String sql = "SELECT * FROM prisoner_information WHERE ssn=?";
 ps = con.prepareStatement(sql);
    ps.setString(1,(request.getParameter("id")));
   rs = ps.executeQuery();
    if(rs.next()) {
        // get the values from the result set and store them in variables
...
%>
    ...
    <tr>
        ...
        <td><input type="text" name="fname" class="form-control" style="border:1px solid gray; color:black; font-size:16px;" value="<%=fname%>" disabled></td>
    </tr>
    <tr>
        <td>Mname<span style="color: red;">*</span></td>
        <td><input type="text" name="mname" class="form-control" style="border:1px solid gray;" value="<%=mname%>" disabled></td>
    </tr>
    <tr>
        <td>Lname<span style="color: red;">*</span></td>
        <td><input type="text" name="lname" class="form-control" style="border:1px solid gray;" value="<%=lname%>" disabled></td>
    </tr>
    <tr>
        <td>Age<span style="color: red;">*</span></td>
        <td><input type="number" name="age" class="form-control " style="border:1px solid gray;" value="<%=age%>" disabled></td>
    </tr>
    <tr>
        <td>Sex<span style="color: red;">*</span></td>
        <td>
            <select name="sex" class="form-control" style="border:1px solid gray;" disabled>
                <option value="male" <% if(sex.equals("male")) {out.print("selected");} %>>Male</option>
                <option value="female" <% if(sex.equals("female")) {out.print("selected");} %>>FeMale</option>
            </select>
        </td>
    </tr>
    <tr>
        <td>Entry date<span style="color: red;">*</span></td>
        <td><input type="text" name="height" class="form-control" style="border:1px solid gray;" value="<%=tsp%>" disabled></td>
    </tr>
    <tr>
        <td>Leave date<span style="color: red;">*</span></td>
        <td><input type="text" name="height" class="form-control" style="border:1px solid gray;" value="<%=tcp%>" disabled></td>
    </tr>
    <tr>
        <td>Transfer To<span style="color: red;">*</span></td>
        <td>
            <select name="transferto" class="form-control" style="border:1px solid gray;">
    ...
    <tr>
        <td>Reason for transfer<span style="color: red;">*</span></td>
        <td><input type="text" name="reason" class="form-control" style="border:1px solid gray;"></td>
    </tr>
    <tr>
        <td>parent phone<span style="color: red;">*</span></td>
        <td><input  name="parent_phone" class="form-control" style="border:1px solid gray;" value="<%=parent_phone %>" disabled></td>
    </tr>
    <tr>
        <td colspan="2">
            <input type="submit" name="update" value="Transfer" class="btn btn-success form-control">
        </td>
    </tr>
</form>
  <%}
...
<%
if (request.getParameter("updatee") != null) {
  ...
  if (newPwd.equals(copassword)) {
    ...
    Statement  st1 = con.createStatement();
      ResultSet rs2=st1.executeQuery(sql6);
...
<tr><td colspan="2"><input type="submit" name="updatee" class="btn btn-info form-control"  value="change password" style="font-size: 20px; height: 45px;"></td></tr>
