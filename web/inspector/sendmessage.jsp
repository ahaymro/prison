...
try {
   ...
   con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");
    ...
    if (request.getParameter("send") != null) {
        ...
        String reciver = request.getParameter("forwho");
        ...
        if (msgtext.isEmpty()) {
            ...
            if (reciver.equals("comissioner")) {
                String sql = "INSERT INTO comissioner_message(sender, message, zone,  date,status) VALUES ('inspector','" + msgtext + "','" + zone + "','" +formattedDate  + "','0')";
                ...
                String sql = "INSERT INTO policeofficer_message(sender, message, zone, status, date) VALUES ('inspector','" + msgtext + "','" + zone + "','0','" + formattedDate + "')";
                query = st.executeUpdate(sql);
                if (query > 0) {
                    out.println("<font style='color:red; font-size:20px; position:absolute;left:600px; top:400px;'> message sent sucessfully</font>");
                } else {
            ...
            }  if (reciver.equals("adminstrator")) {
                String sql = "INSERT INTO admin_message(sender, message, zone, status, date) VALUES ('inspector','" + msgtext + "','" + zone + "','0','" + formattedDate + "')";
                query = st.executeUpdate(sql);
                if (query > 0) {
                    out.println("<font style='color:red; font-size:20px; position:absolute;left:600px; top:400px;'> message sent sucessfully</font>");
                } else {
    ...
    <tr style="text-align: center;"><td style="font-size: 20px; background-color: LightSteelBlue; color: white; font-family: comic sans Ms;">do you know about BG</td>
        ...
        <tr><td class="text-justify" style="font-size: 14px; width: 300px;">  Benishangul-Gumuz  also known as Benshangul/Gumuz, is one of the nine ethnic divisions (kililoch) of Ethiopia. It was previously known as Region 6.The region's capital is Assosa<br>
            ...
            <td style="width: 300px;" class="text-justify" rowspan="3">
              ...
              <form action="" method="post"><!--send message to comissioner and inspector-->
<textarea class="form-control" style="border:2px solid gray; margin-top: 30px; height: 150px;" name="msg" placeholder="please enter the message" required></textarea>
<table class="table"><br>
  <tr><td>send to<span style="color: red;">*</span></td>

<td>
  <select name="forwho" class="form-control" style="border:2px solid gray;">
...
<option value="metekel">Metekel</option>
  ...
  <tr><td colspan="2"><input type="submit" name="send" value="send" class="btn btn-success form-control" style="background-color: #3e8e41;color: white;"onclick="return confirmation ();"></td></tr>
