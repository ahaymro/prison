...
<%
  ...
  String sql3 = "SELECT * FROM prisoner_information WHERE ssn=?";
...
<table class="table  table-striped" style="color: black; font-size: 20px; font-family: comic sans ms;">
  <tr><td>ID_no</td><td><%= rs.getString("ssn") %></td></tr>
  <tr><td>Full Name</td><td><%= rs.getString("fname") %> <%= rs.getString("lname") %></td></tr>
   <tr><td>sex</td><td><%= rs.getString("sex") %></td></tr>
   <tr><td>age</td><td><%= rs.getString("age") %></td></tr>
   <tr><td>height</td><td><%= rs.getString("height") %>&nbsp;meter</td></tr>
   <tr><td>parent_phone</td><td><%= rs.getString("parent_phone") %></td></tr>
   <tr><td>region</td><td><%= rs.getString("region") %></td></tr>
   <tr><td>dorm number</td><td><%= rs.getString("dorm_no") %></td></tr>
   <tr><td>Entry date</td><td><%= rs.getString("tsp") %></td></tr>
   <tr><td>release date</td><td><%= rs.getString("tcp") %></td></tr>
...
if (request.getParameter("update") != null) {
  ...
  if (newPwd.equals(copassword)) {
    String sql7="SELECT * FROM account WHERE username='"+username+"' AND password='"+current+"'";
