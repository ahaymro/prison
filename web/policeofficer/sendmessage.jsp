...
try {
   ...
   con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");
    st = con.createStatement();
    String policeofficer_logged = (String) session.getAttribute("policeofficer_logged");
    if (policeofficer_logged== null) {
       // response.sendRedirect("../login.jsp");
    }
    if (request.getParameter("sendddd") != null) {
        ...
        if (msgtext.isEmpty()) {
            ...
            if (reciver.equals("comissioner")) {
                String sql = "INSERT INTO comissioner_message(sender, message, zone,  date,status) VALUES ('policeofficer','" + msgtext + "','" + zone + "','" +formattedDate  + "','0')";
                ...
                String sql = "INSERT INTO inspector_message(sender, message, zone, status, date) VALUES ('policeofficer','" + msgtext + "','" + zone + "','0','" + formattedDate + "')";
                query = st.executeUpdate(sql);
                if (query > 0) {
                    out.println("<font style='color:red; font-size:20px; position:absolute;left:600px; top:400px;'> message sent sucessfully</font>");
                } else {
                ...
                String sql = "INSERT INTO admin_message(sender, message, zone, status, date) VALUES ('policeofficer','" + msgtext + "','" + zone + "','0','" + formattedDate + "')";
                query = st.executeUpdate(sql);
                if (query > 0) {
                    out.println("<font style='color:red; font-size:20px; position:absolute;left:600px; top:400px;'> message sent sucessfully</font>");
                } else {
...
<option value="metekel">Metekel</option>
              ...
              <input type="submit" name="sendddd" value="send" class="btn btn-success form-control" onclick="return confirmation ();">
