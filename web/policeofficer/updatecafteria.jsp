...
<table class="table table-bordered ">
  <tr>
    ...
    <tr style="text-align: center;"><td style="font-size: 20px; background-color: LightSteelBlue; color: white; font-family: comic sans Ms;">do you know about BG</td>
        <td style="font-size: 20px; background-color: LightSteelBlue; color: black; font-family: comic sans Ms;" >update cafteria schedule</td>
        ...
        <tr><td class="text-justify" style="font-size: 14px; width: 300px;">  Benishangul-Gumuz also known as Benshangul/Gumuz, is one of the nine ethnic divisions (kililoch) of Ethiopia. It was previously known as Region 6.The region's capital is Assosa<br>
            ...
            <table class="table table-bordered">
              <tr><th>ordered type</th> <th>time</th> <th></th></tr>
<% 
    String sql = "select * from schedule where orderedtype='kurs' or orderedtype='misa' or orderedtype='erat'";
  rs = st.executeQuery(sql);
  
    while(rs.next()) {
        report++;
        String orderedtype = rs.getString("orderedtype");
        String eating_time = rs.getString("eating_time");
             int id = rs.getInt("id");
%>
...
<%
    ...
    if(report == 0) {
%>
        <font>data not stored!</font>
<%
    }
%>
            </table>
            </td> 
<td style="font-size: 25px; color: red; font-family: agency fb;"> you are login as <br> policeofficer!
<table><tr>
    <%
  Class.forName("com.mysql.jdbc.Driver");
 con = DriverManager.getConnection("jdbc:mysql://localhost:3306/prisoner", "root", "");
